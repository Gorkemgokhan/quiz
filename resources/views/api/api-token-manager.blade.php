<div>
    ...
    <x-jet-form-section submit="createApiToken">
        <x-slot name="title">
            {{ __('API Simgesi Oluştur') }}
        </x-slot>

        <x-slot name="description">
            {{ __('API belirteçleri, üçüncü taraf hizmetlerin sizin adınıza uygulamamızla kimlik doğrulaması yapmasına olanak tanır.') }}
        </x-slot>

        <x-slot name="form">
            <!-- Token Name -->
            <div class="col-span-6 sm:col-span-4">
                <x-jet-label for="name" value="{{ __('Jeton Adı') }}" />
            ...
            @if (Laravel\Jetstream\Jetstream::hasPermissions())
                <div class="col-span-6">
                    <x-jet-label for="permissions" value="{{ __('izinler') }}" />
    ...
    @if ($this->user->tokens->isNotEmpty())
        ...
        <div class="mt-10 sm:mt-0">
            <x-jet-action-section>
                <x-slot name="title">
                    {{ __('API Jetonlarını Yönetin') }}
                </x-slot>

                <x-slot name="description">
                    {{ __('Artık gerekli değilse mevcut jetonlarınızdan herhangi birini silebilirsiniz.') }}
                </x-slot>
                                ...
                                <div class="flex items-center">
                                    @if ($token->last_used_at)
                                        <div class="text-sm text-gray-400">
                                            {{ __('Son kullanılan') }} {{ $token->last_used_at->diffForHumans() }}
                                        </div>
                                    @endif
